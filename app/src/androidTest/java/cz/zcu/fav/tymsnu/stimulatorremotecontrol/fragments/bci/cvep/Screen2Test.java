...
/**
 * Testovací třída pro otestování GUI v c-VEPu 2. screenu
 */
...
public class Screen2Test {
    ...
    public void testCheckControls() throws Exception {
        onView(withId(R.id.cvep_swipe_number_picker_output_count)).check(matches(isDisplayed()));
        onView(withId(R.id.cvep_swipe_number_picker_bit_shift)).check(matches(isDisplayed()));
        onView(withId(R.id.cvep_edit_text_puls_length)).check(matches(isDisplayed()));
        onView(withId(R.id.cvep_seekbar_brightness)).check(matches(isDisplayed()));
        onView(withId(R.id.cvep_button_save_values)).check(matches(isDisplayed()));
